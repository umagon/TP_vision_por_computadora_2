...
\begin{abstract}
Las enfermedades retinales (CNV, DME, Drusen) son causas principales de pérdida visual; la OCT es el estándar de diagnóstico por imagen. En este trabajo se evalúa si la segmentación semántica de capas retinales como preprocesamiento mejora la clasificación automática de patologías en imágenes OCT. Para ello se entrenó un modelo UNet++ (ResNet34) para segmentación de 8 capas retinales, y dos clasificadores ResNet50: uno sobre imágenes originales y otro sobre las máscaras de segmentación generadas. El modelo de segmentación alcanzó un Dice de 0.84 (5-fold CV). El clasificador sobre imágenes originales logró 94\% de accuracy, mientras que el clasificador sobre máscaras de segmentación alcanzó 70\%. Este hallazgo nos indica que la información de textura y contraste de las imágenes originales resulta más discriminativa para la clasificación de patologías que la información puramente estructural de las máscaras de segmentación.
\end{abstract}

\begin{IEEEkeywords}
OCT, segmentación semántica, clasificación de imágenes médicas, deep learning, retina.
\end{IEEEkeywords}
...
La interpretación manual de OCT requiere especialistas, es subjetiva y consume tiempo. Esto impulsa la necesidad de automatización con deep learning.
...
¿La segmentación semántica de capas retinales como paso previo mejora la clasificación de patologías en OCT?. Las contribuciones principales de este trabajo son:
\begin{enumerate}
    \item Un modelo de segmentación semántica de capas retinales basado en U-Net++ con validación cruzada de 5 pliegues.
    \item Un modelo clasificador 1 usando resnet
